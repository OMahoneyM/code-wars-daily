...
function addUsername(arr){   
    arr.forEach(e => e.username = e.firstName.toLowerCase() + e.lastName[0].toLowerCase()+ (new Date().getFullYear() - e.age))
...
function addUsername(arr){   
    return arr.map(e => { 
        e.username = e.firstName.toLowerCase() + e.lastName[0].toLowerCase()+ (new Date().getFullYear() - e.age) 
        return e;
    })
}
console.log(addUsername(list1))