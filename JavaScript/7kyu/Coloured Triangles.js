...
function triangle(row) {
    let result = (row.length > 1) ? '' : row;

    for (let i = 0; i < row.length - 1; i++) {
        (row[i] === row[i + 1]) ? result += row[i] : result += 'RGB'.replace(row[i], '').replace(row[i + 1], '')
    }

    return (result.length > 1) ? triangle(result) : result
}

console.log(triangle("G"), "G")
